...
const pool = new Pool({
  connectionString: 'postgresql://projectflow:projectflow_secret_2024@localhost:5432/projectflow',
});
...
async function seed() {
  ...
  const memberCount = await pool.query(
    `INSERT INTO project_members (project_id, user_id, role, allocation_percentage)
     VALUES
       ($1, $1, 'project_manager', 100),
       ($1, $3, 'team_lead', 100),
       ($1, $4, 'developer', 100),
       ($1, $5, 'developer', 80),
       ($1, $6, 'qa', 100),
       ($2, $2, 'project_manager', 100),
       ($2, $3, 'developer', 50),
       ($2, $6, 'qa', 50),
       ($3, $1, 'project_manager', 100),
       ($3, $4, 'developer', 100),
       ($3, $5, 'developer', 100),
       ($4, $7, 'pmo', 100),
       ($4, $2, 'project_manager', 50),
       ($4, $3, 'team_lead', 50)
  ...
  console.log(`  ✅ ${memberCount.rowCount} members added`);
  ...
  const sprintData = [
    { project: 0, name: 'Sprint 1 - Foundation', goal: 'Setup core architecture and database', status: 'completed', start: '2026-01-15', end: '2026-01-29' },
    { project: 0, name: 'Sprint 2 - Core Modules', goal: 'Implement user management and auth', status: 'completed', start: '2026-01-30', end: '2026-02-13' },
    { project: 0, name: 'Sprint 3 - Inventory', goal: 'Build inventory management module', status: 'active', start: '2026-02-14', end: '2026-02-28' },
    { project: 0, name: 'Sprint 4 - Sales & Purchasing', goal: 'Implement sales and purchasing', status: 'planning', start: '2026-03-01', end: '2026-03-15' },
    { project: 2, name: 'Sprint 1 - UI/UX', goal: 'Design and frontend setup', status: 'active', start: '2026-02-01', end: '2026-02-15' },
  ];
  ...
  for (const s of sprintData) {
    ...
    sprintIds.push(id);
    await pool.query(
      `INSERT INTO sprints (id, project_id, name, goal, status, start_date, end_date, created_by)
  ...
  const taskData = [
    ...
    { project: 0, sprint: 3, title: 'Reporting Module', status: 'backlog', priority: 'low', assignee: 5, type: 'story', points: 8, hours: 20 },
    ...
    { project: 2, sprint: 4, title: 'Performance Testing', status: 'backlog', priority: 'medium', assignee: 6, type: 'task', points: 5, hours: 12 },
  ...
  console.log('\n⚠️ Creating risks...');
  const riskData = [
    { project: 0, title: 'Key developer may leave the project', prob: 3, impact: 5, level: 'high', status: 'identified' },
  ...
  for (const r of riskData) {
    await pool.query(
      ...
      [randomUUID(), projectIds[r.project], r.title, `Risk: ${r.title}`, r.prob, r.impact, r.level, r.status, userIds[1], 'Mitigation plan for ' + r.title]
    );
  }
  ...
  for (const ts of timesheetData) {
    await pool.query(
      `INSERT INTO timesheets (id, user_id, project_id, date, hours, description, activity_type, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [randomUUID(), userIds[ts.user], projectIds[ts.project], ts.date, ts.hours, `${ts.activity} work`, ts.activity, ts.status]
    );
  }
  console.log(`  ✅ ${timesheetData.length} timesheet entries created`);
  ...
  console.log('\n📅 Creating meetings...');
  const meetingData = [
    { project: 0, title: 'Sprint Planning - ERP Implementation', type: 'sprint_planning', start: '2026-06-20T09:00:00+07:00', end: '2026-06-20T11:00:00+07:00', organizer: 1 },
    ...
    { project: 1, title: 'Stakeholder Update - Digital Transformation', type: 'stakeholder', start: '2026-06-25T10:00:00+07:00', end: '2026-06-25T11:00:00+07:00', organizer: 2 },
  ];

  for (const m of meetingData) {
    await pool.query(
      `INSERT INTO meetings (id, project_id, title, meeting_type, start_time, end_time, organizer_id, status)
  ...
  for (let i = 1; i < userIds.length; i++) {
    await pool.query(
      `INSERT INTO notifications (id, user_id, type, title, message, reference_type, is_read)
       VALUES ($1, $2, 'system', 'Welcome to ProjectFlow', 'Your account has been created. Welcome aboard!', 'system', $3)`,
      [randomUUID(), userIds[i], i % 2 === 0]
    );
  }
  console.log(`  ✅ ${users.length - 1} notifications created`);
  ...
  await pool.query(
    `INSERT INTO audit_logs (id, user_id, action, entity_type, entity_id, new_values, ip_address)
     VALUES
       ($1, $2, 'create', 'project', $3, '{"name":"ERP Implementation"}', '192.168.1.100'::inet),
       ($1, $2, 'create', 'project', $4, '{"name":"Digital Transformation"}', '192.168.1.100'::inet),
       ($1, $2, 'create', 'task', $5, '{"title":"Setup Database Schema"}', '192.168.1.100'::inet)`,
    [randomUUID(), userIds[0], projectIds[0], projectIds[1], projectIds[2]]
  );
  ...
  const budgetData = [
    { project: 0, category: 'Development', desc: 'Sprint 1-2 Development', planned: 100000000, actual: 95000000, type: 'expense', date: '2026-02-28' },
    { project: 0, category: 'Infrastructure', desc: 'Server and hosting setup', planned: 50000000, actual: 48000000, type: 'expense', date: '2026-01-31' },
    { project: 0, category: 'Revenue', desc: 'Phase 1 Payment', planned: 200000000, actual: 200000000, type: 'income', date: '2026-03-15' },
    { project: 1, category: 'Consulting', desc: 'Business analysis', planned: 80000000, actual: 60000000, type: 'expense', date: '2026-04-30' },
    { project: 2, category: 'Development', desc: 'Frontend development', planned: 120000000, actual: 75000000, type: 'expense', date: '2026-06-30' },
  ];

  for (const b of budgetData) {
    await pool.query(
      `INSERT INTO budget_entries (id, project_id, category, description, planned_amount, actual_amount, date, entry_type, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [randomUUID(), projectIds[b.project], b.category, b.desc, b.planned, b.actual, b.date, b.type, userIds[1]]
    );
  }
  console.log(`  ✅ ${budgetData.length} budget entries created`);

  console.log('\n🎉 Seed completed successfully!');
  console.log('\n📊 Summary:');
  console.log('  - 9 users (1 admin + 8 team members)');
  console.log('  - 4 projects');
  console.log('  - 14 project members');
  console.log('  - 5 sprints');
  console.log('  - 16 tasks');
  console.log('  - 5 issues');
  console.log('  - 4 risks');
  console.log('  - 5 timesheet entries');
  console.log('  - 3 meetings');
  console.log('  - 8 notifications');
  console.log('  - 5 budget entries');
  console.log('\n🔑 Default login: admin@projectflow.com / admin123');
